// On importe 'writable' pour créer un store réactif (partagé entre les composants)
import { writable } from 'svelte/store';

// On importe 'browser' pour vérifier si le code s'exécute dans un navigateur
// (important car 'localStorage' n'existe pas côté serveur)
...
function saveToStorage(map) {
  ...
  try {
    const object = Object.fromEntries(map); // On convertit la Map en objet (Object.entries inverse)
    localStorage.setItem('booklistStatus', JSON.stringify(object));
  } catch (error) {
...
// Fonction pour charger le cache seulement lorsque c'est nécessaire (=lazy loading), plus rapide et plus léger 
...
// Fonction helper pour mettre à jour le statut d'un livre (fonction helper = petite fonction réutilisable pour éviter la répétition de code)
